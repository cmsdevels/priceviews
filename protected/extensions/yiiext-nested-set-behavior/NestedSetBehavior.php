...
class NestedSetBehavior extends CActiveRecordBehavior
{
     ...
     * @param boolean $descendantsDeletion if we need to call delete method for all descendants
	 * @return boolean whether the deletion is successful.
	 */
	public function delete($descendantsDeletion = false)
	{
		$owner=$this->getOwner();
		...
		try
		{
			...
			else
			{
                ...
                if ($descendantsDeletion==true) {
                    $children = $owner->children()->findAll();
                    $result = true;
                    foreach ($children as $child) {
                        $result = $child->deleteNode(true);
                        if ($result==false) {
                            break;
                        }
                    }
	 ...
	 * @return boolean whether the deletion is successful.
	 */
	public function deleteNode($descendantsDeletion = false)
	{
		return $this->delete($descendantsDeletion);
	}
