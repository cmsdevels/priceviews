...
(function($) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
        ...
        getItem: function(options) {
            ...
            id = ($(options.item).attr(o.attribute || 'id')).match(o.expression || (/(.+)[-=_](.+)/))[2];

            var parentItem = $(options.item).parent(o.listType).parent(o.items);
            if (parentItem[0]) {
                pid = (parentItem.attr(o.attribute || 'id'))
                    .match(o.expression || (/(.+)[-=_](.+)/))[2];
            } else {
                pid = o.rootID;
            }

            if ($(options.item).prev()[0]) {
                prev_id = ($(options.item.prev()).attr(o.attribute || 'id')).match(o.expression || (/(.+)[-=_](.+)/))[2];
            } else {
                prev_id = pid
            }

            return { id: id, parent_id: pid, prev_id: prev_id };
        },
