...
class JsTreeBehavior extends CBehavior
{
                ...
                if ($previous_node_id == 'false' && $next_node_id == 'false') {
                    if ($moved_node->moveAsFirst($new_parent)) {
                        echo json_encode(array(
                            'success' => true,
                            'viewUrl'=>$moved_node->getViewItemUrl()
                        ));
                    ...
                    if ($moved_node->moveAsFirst($new_parent)) {
                        echo json_encode(array('success' => true, 'viewUrl'=>$moved_node->getViewItemUrl()));
        ...
        //else,move it as a new Root
            ...
            if (!$moved_node->isRoot()) {
                ...
                if ($moved_node->moveAsRoot()) {
                    ...
                    echo json_encode(array('success' => false, 'viewUrl'=>$moved_node->getViewItemUrl()));
                }
            } //else if moved/copied node is Root
            else {
                echo json_encode(array('success' => false, 'message' => 'Node is already a Root.Roots are ordered by primary key.', 'viewUrl'=>$moved_node->getViewItemUrl()));
            }
        }
    }
    ...
    public function printULTree()
    {
        ...
        $categories = CActiveRecord::model($this->modelClassName)->findAll(array('order' => $this->order));
        $level = 0;
