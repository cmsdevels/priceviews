...
EOD;
        ...
        Yii::app()->clientScript->registerScript(__CLASS__ . 'structure_js', "
            $(document).tooltip({
                selector: '.jstree-structure span.actions>span',
                trigger: 'hover'
            });

            $(document).on('click', '.jstree-structure span.actions>span.view, .jstree-structure span.actions>span.edit', function () {
                window.location.href = $(this).data('href');
            });

            $(document).on('click', '.jstree-structure span.actions>span.delete', function () {
                var href = $(this).data('href');

                if (confirm('".Yii::t('core/admin', 'Are you sure you want to delete the page?')."')) {

                    $.post(href, {}, function () {
                        $('#".$this->jstree_container_ID."').jstree('refresh');
                    });
                }
            });
