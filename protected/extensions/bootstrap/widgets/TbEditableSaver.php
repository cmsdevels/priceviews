...
class TbEditableSaver extends CComponent
{
    ...
    */
     ...
     *
     */
    public function update($isNode=false)
    {
        ...
        if ($this->beforeUpdate()) {
            //saving (only changed attributes)
            if ($isNode)
                $save = $this->model->saveNode(false, $this->changedAttributes);
            else
                $save = $this->model->save(false, $this->changedAttributes);
            if ($save) {
                $this->afterUpdate();
            } else {
